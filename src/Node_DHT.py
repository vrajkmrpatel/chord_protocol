    ...
    def start(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                conn, addr = s.accept()
                t = threading.Thread(
                    target=self.serve_requests, args=(conn, addr))
                t.start()
        ...
        if self.successor.id != self.id:
            ...
            for key_value in data.split(':'):
                if len(key_value) > 1:
                    ...
                    self.data_store.data[key_value.split(
                        '|')[0]] = key_value.split('|')[1]
    ...
    def stabilize(self):
        ...
        while True:
            ...
            if result == "None" or len(result) == 0:
                self.request_handler.send_message(
                    self.successor.ip, self.successor.port, "notify|" + str(self.id) + "|" + self.nodeinfo.__str__())
                continue
...
class FingerTable:
    ...
    def print(self):
        ...
        for index, entry in enumerate(self.table):
            if entry[1] is None:
                print('Entry: ', index, " Interval start: ",
                      entry[0], " Successor: ", "None")
            else:
                print('Entry: ', index, " Interval start: ",
                      entry[0], " Successor: ", entry[1].id)
